<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-2xl font-bold text-gray-900">{{ __('Edit Permission') }}</h2>
                <p class="mt-1 text-sm text-gray-600">Update system permission</p>
            </div>
        </div>
    </x-slot>
        ...
        <div class="bg-white rounded-lg shadow-soft border border-gray-200 p-6">
            <form action="{{ route('admin.permissions.update', $permission) }}" method="post" class="space-y-6">
                ...
                <div>
                    ...
                    <x-text-input id="name" name="name" value="{{ old('name', $permission->name) }}" class="mt-1 block w-full" required />
                ...
                <div class="flex items-center gap-3 pt-4 border-t border-gray-200">
                    ...
                    <x-primary-button>Update Permission</x-primary-button>
                </div>
            </form>
        </div>
    </div>
