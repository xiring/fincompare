<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-2xl font-bold text-gray-900">{{ __('Edit Page') }}</h2>
                <p class="mt-1 text-sm text-gray-600">Update CMS page information</p>
            </div>
        </div>
    </x-slot>
    ...
    <div class="max-w-4xl">
        <div class="bg-white rounded-lg shadow-soft border border-gray-200 p-6">
            <form action="{{ route('admin.cms-pages.update', $cms_page) }}" method="post" class="space-y-6">
                @csrf
                @method('PATCH')
                    ...
                    <div>
                        ...
                        <select id="status" name="status" class="mt-1 block w-full border-gray-300 rounded-lg shadow-sm focus:border-primary-500 focus:ring-primary-500 text-sm">
                            @foreach(['draft','published'] as $st)
                                <option value="{{ $st }}" {{ old('status', $cms_page->status)===$st?'selected':'' }}>{{ ucfirst($st) }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                ...
                <div>
                    ...
                    <textarea id="seo_description" name="seo_description" rows="3" class="mt-1 block w-full border-gray-300 rounded-lg shadow-sm focus:border-primary-500 focus:ring-primary-500 text-sm">{{ old('seo_description', $cms_page->seo_description) }}</textarea>
                </div>
                ...
                <div class="flex items-center gap-3 pt-4 border-t border-gray-200">
                    <a href="{{ route('admin.cms-pages.index') }}" class="inline-flex items-center justify-center px-4 py-2.5 bg-white border border-gray-300 rounded-lg font-medium text-sm text-gray-700 hover:bg-gray-50 transition-colors">
                        Cancel
                    </a>
