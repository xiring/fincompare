<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-2xl font-bold text-gray-900">{{ __('Create Blog Post') }}</h2>
                <p class="mt-1 text-sm text-gray-600">Add a new blog post</p>
            </div>
        </div>
    </x-slot>
            ...
            <form action="{{ route('admin.blogs.store') }}" method="post" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <x-text-input id="category" name="category" value="{{ old('category') }}" class="mt-1 block w-full" />
                    </div>
                    <div>
                ...
                <div>
                    ...
                    <x-text-input id="featured_image" name="featured_image" type="url" value="{{ old('featured_image') }}" class="mt-1 block w-full" />
                </div>
                ...
                <div>
                    ...
                    <textarea id="seo_description" name="seo_description" rows="3" class="mt-1 block w-full border-gray-300 rounded-lg shadow-sm focus:border-primary-500 focus:ring-primary-500 text-sm">{{ old('seo_description') }}</textarea>
                </div>

                <div>
                    <x-input-label for="tags" value="Tags (comma-separated or JSON array)" />
                    <x-text-input id="tags" name="tags" value="{{ old('tags') }}" class="mt-1 block w-full" />
                </div>
