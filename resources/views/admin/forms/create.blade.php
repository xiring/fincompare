<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-2xl font-bold text-gray-900">{{ __('Create Form') }}</h2>
                <p class="mt-1 text-sm text-gray-600">Add a new dynamic form</p>
            </div>
        </div>
    </x-slot>
    ...
    <div class="max-w-4xl">
        <div class="bg-white rounded-lg shadow-soft border border-gray-200 p-6">
            <form action="{{ route('admin.forms.store') }}" method="post" class="space-y-6" id="form-form">
                @csrf
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="3" class="mt-1 block w-full border-gray-300 rounded-lg shadow-sm focus:border-primary-500 focus:ring-primary-500 text-sm">{{ old('description') }}</textarea>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select id="type" name="type" class="mt-1 block w-full border-gray-300 rounded-lg shadow-sm focus:border-primary-500 focus:ring-primary-500 text-sm" required>
                            <option value="pre_form" {{ old('type', 'pre_form')==='pre_form'?'selected':'' }}>Pre Form</option>
                            <option value="post_form" {{ old('type')==='post_form'?'selected':'' }}>Post Form</option>
                        </select>
    ...
    <script>
        ...
        function toggleOptions(index) {
            const select = document.getElementById(`inputs_${index}_type`);
        ...
        document.getElementById('form-form').addEventListener('submit', function(e) {
            const inputs = this.querySelectorAll('[id^="inputs_"][id$="_options"]');
            inputs.forEach(input => {
                const name = input.name.replace('_text', '');
                const text = input.value.trim();
                if (text) {
                    const options = text.split('\n').filter(line => line.trim()).map(line => line.trim());
                    const hiddenInput = document.createElement('input');
                    hiddenInput.type = 'hidden';
                    hiddenInput.name = name.replace('options_text', 'options');
